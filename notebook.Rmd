...
In clinical or epidemiological research it is common for a clinical biomarker to be repeatedly measured over time on a given patient. In addition, it is common for time-to-event data, such as the patient-specific time from a defined origin (e.g. time of diagnosis of a disease) until a terminating clinical event such as death or disease progression to also be collected. For example, the figure below shows observed longitudinal measurements (i.e. observed "trajectories") of log serum bilirubin for a small sample of primary bilary cirhosis patients. From the figure, we can observe between-patient variation in the longitudinal trajectories with some patients increasing over time, others decreasing, and some remaining stable. Moreover, there is variation between patients in terms of the frequency and timing of the longitudinal measurements. From a clinical perspective, we may be interested in asking whether the between-patient variation in the log serum bilirubin trajectories provides information that helps us understand differences in patient risk for some clinical event of interest, say, death. For example, we might be interested in the prognostic value of changes in log serum bilirubin, in terms of it's ability to help estimate patient-specific mortality risk. Alternatively, from an epidemiological perspective we might have a desire to understand the potential for an etiological association between changes in log serum bilirubin and mortality. Joint modelling approaches provide us with a framework under which we can begin to answer these types of clinical and epidemiological questions. 

```{r traj_figure}
patient_labels <- paste("Patient", 1:6)
ggplot() + 
  geom_line(aes(y = logBili, x = year), 
            pbcLong %>% filter(id %in% c(25,31:33,36,38:40))) + 
  facet_wrap(~ id, ncol = 4, labeller = labeller(id = patient_labels)) +
  ylab("Log serum bilirubin") +
  xlab("Time (years)")
```

More formally, the motivations for undertaking a joint modelling approach of longitudinal and time-to-event data might include one or more of the following:
