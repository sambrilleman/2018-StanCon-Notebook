...
To see this, we can first install the development version of **rstanarm** with the `stan_jm` modelling function:
\
```{r install_rstanarm, message=FALSE}
if (!require(devtools))
  install.packages("devtools")
if (!require(rstanarm) || !exists("stan_jm", "package:rstanarm")) 
  devtools::install_github(
    "sambrilleman/rstanarm", dependencies = TRUE,
    args = "--preclean", build_vignettes = FALSE)
library(rstanarm)
```

and then fit our model again, this time using `stan_jm` with the customary R formula syntax and data frames:
\
